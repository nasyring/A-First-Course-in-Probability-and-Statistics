...
```{r, echo = F, eval = T}
exp1 <- function(x) dexp(x,1)
dlnorm1 <- function(x) dlnorm(x,1,1)


curve(exp1, 0,5, xlab = 'x', ylab = 'Density')
curve(dlnorm1, 0,5,add = T, col = 'blue')


x1 <- colMeans(matrix(rexp(30*1000,1),30,1000))
x2 <- colMeans(matrix(rlnorm(30*1000,1,1),30,1000))

par(mfrow = c(1,2))

hist((x1-mean(x1))/(sd(x1)), xlab = 'Exponential(1) sample means', main = '')
hist((x2-mean(x2))/(sd(x2)), xlab = 'Lognormal(1,1) sample means', main = '')
...
x1 <- colMeans(matrix(rcauchy(50*1000),50,1000))
hist((x1-mean(x1))/(sd(x1)), xlab = 'Cauchy sample means', main = '', freq = F)
