...
The same Chi-squared test used above comparing observed awith expected cell counts is used to test the "goodness of fit" of a model to a data set.  This test is most often used with discrete data, but can be used with continuous data by "binning" or discretizing continuous data in the same fashion as a histogram.  For tabulated data the goodness of fit test statistic has df $k-p$ where $k$ is the number of cells in the table and $p$ is the number of estimated parameters in the null hypothesis.  <br><br>
...
```r
1-pchisq(sum(((x - mean(x))^2)/mean(x)),11-1)
```
